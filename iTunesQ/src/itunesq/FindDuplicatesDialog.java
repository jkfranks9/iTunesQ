...
/**
 * Class that handles the find duplicates dialog. This dialog allows the user
 * to find exact song duplicates, or fuzzy duplicates based on track attribute
...
public class FindDuplicatesDialog
{
			...
			throws IOException, SerializationException
	{
		...
		duplicatesDoneButton.getButtonPressListeners().add(new ButtonPressListener() 
        {
            ...
            public void buttonPressed(Button button)
            {
				...
				matchSpec.set(MatchCriteria.EXACT.ordinal(), 
						duplicatesSpecExactCheckbox.isSelected());
				...
				if (matchSpec.get(MatchCriteria.EXACT.ordinal()) == false)
				{
					...
					matchSpec.set(MatchCriteria.KIND.ordinal(), 
							duplicatesSpecKindCheckbox.isSelected());
					matchSpec.set(MatchCriteria.DURATION.ordinal(), 
							duplicatesSpecDurationCheckbox.isSelected());
					matchSpec.set(MatchCriteria.YEAR.ordinal(), 
							duplicatesSpecYearCheckbox.isSelected());
					matchSpec.set(MatchCriteria.RATING.ordinal(), 
							duplicatesSpecRatingCheckbox.isSelected());
				}
				...
				/*
				 * Get the duplicates map. This maps track name to all the IDs with that name.
				 */
				 ...
				 */
				Iterator<String> duplicatesMapIter = duplicatesMap.iterator();
				while (duplicatesMapIter.hasNext())
				{
					String dupName = duplicatesMapIter.next();
					...
					Iterator<Integer> dupIDsIter = dupIDs.iterator();
					while (dupIDsIter.hasNext())
					{
					...
					/*
					 * Now create the duplicate tracks (if any) based on the match criteria.
					 */
				...
				else
				{
					Alert.alert(MessageType.INFO, 
							StringConstants.ALERT_NO_TRACKS, owningWindow);
				}
		...
		/*
		 * Listener to enable or disable fuzzy search criteria based on the exact match checkbox.
		 */
	...
	/*
	 * Given a list of possible duplicate tracks, determine which are actual duplicates based on
	 * the match criteria.
	 */
	private void addDuplicateTracks(String dupName, List<Track> dupTracksForName, 
			List<Track> allDupTracks)
	{
		...
		for (int i = 0; i < dupsLen; i++)
		{
			...
			for (int j = i + 1; j < dupsLen; j++)
			{
				...
				if (matchTracks(track1, track2) == true)
				{
					...
					/*
					 * Since we have nested loops, one or both tracks might already be in the list, 
					 * so we only add them if not.
					 * 
					 * NOTE: Some hideous Pivot code to deal with. The indexOf method uses a binary
					 * search if the array list has an associated comparator, which is indeed the case.
					 * This means our comparator's compare method is used, which only compares the 
					 * name, meaning it finds a match based only on name, which is not what we want.
					 * So, we save, clear, and restore the comparator so the entire object gets 
					 * compared.
					 */
	...
	private boolean matchTracks (Track track1, Track track2)
	{
		...
		/*
		 * Duration is special: since it's in milliseconds, the actual value for two tracks
		 * might be slightly different, even though the time display in MM:SS format is the
		 * same. So convert the time to MM:SS for comparison.
		 */
		...
		if (matchSpec.get(MatchCriteria.EXACT.ordinal()) == true)
		{
			...
			if (   isEqual(track1.getArtist(), track2.getArtist())
				&& isEqual(track1.getAlbum(), track2.getAlbum())
				&& isEqual(track1.getKind(), track2.getKind())
				&& isEqual(duration1, duration2)
	...
	/*
	 * Check equality of two track attributes. One or both might be null. Some attributes are
	 * strings and some are primitive int types.
	 */
    		...
    		throws IOException, SerializationException
    {
        ...
        findDuplicatesDialog = (Dialog)dialogSerializer.readObject(getClass().
				getResource("findDuplicatesDialog.bxml"));
